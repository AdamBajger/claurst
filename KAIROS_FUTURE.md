...
- **LiveSession CLI bootstrap.** ~~Deferred~~ → **shipped:** `cli/main.rs::run_interactive` builds `LiveSession::with_projects(settings, cwd, cost_tracker, perm_manager, project_registry)` once at startup. `CommandContext.live_session: Option<SharedLiveSession>` and `ToolContext.task_tracker`/`event_log` flow through every spawn site. The named-command headless fast path and ACP server path keep `live_session: None` (pre-session) — slash commands that need it gracefully degrade with a "(pre-session path)" message.
- **`/project list|switch|create|show` slash commands.** ~~Deferred~~ → **shipped** (with caveats): `ProjectCommand` in `crates/commands/src/lib.rs`. `list` / `show` / `switch <name>` / `create <name> --root <path>` all wired against `live_session.runtime.project_registry`. Atomic permission-rule + MCP swap on `switch` is **not yet** wired (see Project rule routing below).
- **`AgentConfig.kairos_policy` field.** ~~Deferred~~ → **shipped:** field is `Option<KairosPermissionPolicy>` with serde default. `KAIROS_PERMISSION_POLICY` env still governs when the per-agent field is `None`.
- **`TaskSource` on `PendingPermission`.** ~~Deferred~~ → **shipped:** `PendingPermission.source: Option<TaskSource>` plus `register_pending_with_source` and `pending_snapshot()` for `/permissions list --by-source` filter views. Legacy `register_pending` stores `None`.
- **`evaluate()` consults `KairosPermissionPolicy`.** ~~Deferred~~ → **shipped:** `PermissionManager::evaluate_with_source(req, source, policy)` is the policy-aware path — non-foreground sources collapse `Ask` to `Deny` (Reject), `Allow` (AutoAllowRead + read-only), or fall through (DeferToUser). Foreground sources unaffected. Existing `evaluate()` retained as the policy-blind fast path.
...
- **`/permissions` slash command (list/revoke/grant).** ~~Partially shipped~~ → **fully shipped:** `list`, `revoke <id>` and `grant <subject> <scope> [allow|deny]` all wired against `live_session.runtime.permissions`. Subject grammar: `tool:NAME | tool-input:NAME[:CONTAINS] | path:PATH[:read|write|any] | url:GLOB | cmd:bash|powershell|any:GLOB`. Scope grammar: `once | session | forever | project[:NAME]` (project falls back to active project name). Forever-scoped grants persist the rule shape (without subject) into `Settings.permission_rules` for restart survival; project-scoped grants live in `PermissionManager.project_rules` and are not yet mirrored to the on-disk `ProjectConfig.permission_rules` at runtime — disk-mirror for project rules is the remaining gap.
- **`PermissionScope::Project { name }` rule routing.** ~~Pending~~ → **shipped end-to-end:** `PermissionManager` gained `project_rules: BTreeMap<String, Vec<PermissionRule>>` + `active_project: Option<String>`. `add_rule` routes `Project { name }` rules into the named bucket; `evaluate` consults the active bucket between persistent and session rules; `list_rules` includes them; `remove_rule_by_id` searches them. `LiveSession::switch_project` loads `ProjectConfig.permission_rules` into the bucket and sets the active marker; `clear_active_project` drops it. Disk mirror: `LiveSession::persist_project_rule` + `persist_remove_project_rule` write through to `~/.claurst/projects/<name>.json`; `/permissions grant ... project:N` and `/permissions revoke <id>` invoke them so changes survive restart. `SerializedPermissionRule` extended with `id` + `subject` (additive, serde-defaulted) so project rules round-trip with full fidelity.
- **Phase 9.5 producer sites beyond `execute_agent_run`.** `TaskTracker` ships and `execute_agent_run` registers `AgentRunTask`. Tool-dispatch and `AgentTool` `SubagentTask` not yet wired as tracker rows (they emit `ToolCall` events but no tracker entries). Cron-tick uses `execute_agent_run`'s `AgentRunTask` registration — no separate `CronTickTask` needed.
- **`/tasks` + `/stop all` slash commands.** ~~Deferred~~ → **shipped:** `TasksCommand` (`list` / `show <id>` / `cancel <id>`) and `StopCommand` (`/stop all [--yes]`) both consult `ctx.task_tracker`. Confirmation gate enforced unless `--yes`.
- **`StopAllTasks` tool.** ~~Deferred~~ → **shipped:** `crates/tools/src/stop_all_tasks.rs`. Constructed with a `TaskTracker` clone; calls `cancel_all`. `PermissionLevel::Dangerous` so even with explicit allowlist the user must approve at request time. Default agent tool set must still NOT register it — opt-in via `tools.allowlist`.
- **Tracker → event-log lifecycle hooks.** Currently each producer site emits its own `BackgroundStart` / `BackgroundFinish` directly (single-producer per event). A central tracker-driven emitter is **not** wired and is unlikely to be needed unless we add many more producer sites; revisit if duplication risk grows.
- **Phase 10 producer sites beyond `execute_agent_run`.** ~~Pending: snapshot loader, panic boundary~~ → **fully shipped:** `BackgroundStart` / `BackgroundFinish` from `execute_agent_run`; `ToolCall { Started → Succeeded/Failed }` from `query::execute_tool`; `CronFired` from `cron_scheduler`; `PermissionRequested` + `PermissionDecided` from `PermissionManager` via `set_event_log` injection; `TurnStart` / `TurnEnd` from `cli/main.rs`; `AgentSpawned` from `cron_scheduler` (named-agent crons) + `cli/main.rs::spawn_named_agent_run` (`/agent run`); `SnapshotPartialLoad` from CLI bootstrap when `ProjectRegistry::load_from_dir_with_failures` reports skipped files; `TaskPanicked` from `query::background_runner::spawn_agent_run` (wraps inner future in `futures::FutureExt::catch_unwind`, deregisters task on panic, surfaces `AgentRunResult { is_error: true }` so the TUI sees it instead of a silent worker death).
- **`/activity` modal + status-line wiring.** `/activity [N] [--source <kind>]` ships as a text-rendering slash command (not a scrollable modal yet). Status-line wiring (`EventLog::most_recent` → avatar line) ~~not yet~~ → **shipped (welcome-screen hook):** `App.recent_activity: Option<String>` is refreshed each tick from `event_log.most_recent()` in the CLI loop and rendered on the welcome-screen "Recent activity" line. Continuous below-prompt avatar status-line surfacing remains pending.
- **Graceful-shutdown JSONL flush.** ~~Deferred~~ → **shipped:** `event_log.flush_to_jsonl()` is called in `run_interactive` after the main loop exits. Panic-boundary flush (TUI panic handler) still pending.
- **Phase 11 `/agent` user-facing subcommands.** ~~Partially shipped~~ → **fully shipped:** `AgentCommand` now supports `list`, `show <name>`, `create <name>`, `delete <name>`, `persist <name>`, `run <name> <prompt>`, plus sugar `/agent <name>` (= show) and `/agent <name> <prompt>` (= run, when name resolves). Spawn returns the new `CommandResult::SpawnNamedAgent` variant; the CLI loop intercepts it via `spawn_named_agent_run` (parallel to `spawn_background_slash_command`) which resolves `AgentConfig` via `LiveSession::resolve_agent_config`, emits `AgentSpawned`, and dispatches `execute_agent_run` with `AgentRunSource::SlashCommand { name = "agent:<name>" }` for /activity attribution.
